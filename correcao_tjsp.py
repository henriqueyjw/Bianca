...
name_report = 'tjrs_correto.csv'
# ini_path = sys.argv[1]
# final_path = sys.argv[2]
# name_report = sys.argv[3]
...
dicionario_expressoes_1 = {
    ...
    'leasing': r'leasing',
    'classe_proc': r'classe:\s+procedimento\scomum',
    'recisao': r'recis[aã]o',
    'compra_venda': r'compra\se\svenda|venda\se\scompra',
    'acao_cobranca':r'a[cç][aã]\sde\scobran[cç]a',
    'bradesco': r'bradesco',
    ...
    if f[-3:] == 'csv':
        ...
        for _, row in df.iterrows():
            ...
            if flag:
                ...
                numero_atual = re.search('\d{1,7}\-\d{2}\.\d{4}\.\d\.\d{2}\.\d{4}', row['texto_publicacao'])
                numero_antigo = re.search('\d{4}\.\d{2}\.\d{2}\.\d{6}\-\d{1}', row['texto_publicacao'])
                if numero_atual:
